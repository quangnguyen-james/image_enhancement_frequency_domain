...
if __name__ == "__main__":
    # Đọc ảnh
    image = cv2.imread("TestPattern.png", 0)
    ...
    for i in range(P):
        dft_col[i] = DFT1D(F_xy_p[i])
    # DFT chiều Q - theo hàng
    for j in range(Q):
        dft_row[:, j] = DFT1D(dft_col[:, j])
    ...
    for i in range(P):
        idft_col[i] = IDFT1D(G_uv[i])
    # Chuyển đổi DFT ngược theo chiều Q - theo hàng
    for j in range(Q):
        idft_row[:, j] = IDFT1D(idft_col[:, j])
